...
class Api::V1::WebsitesController < ApplicationController
  def index
    ...
    products.each do |product|
      amazon_prices = Price.where("product_id = ? AND website_id = ?", product.id, 1)
      last_amz_price = amazon_prices.last
      walmart_prices = Price.where("product_id = ? AND website_id = ?", product.id, 2)
      last_walmart_price = walmart_prices.last
